...
| Playwright (full suite, `PLAYWRIGHT_CHROMIUM_PATH=/opt/pw-browsers/chromium-1194/chrome-linux/chrome`) | **471 pass, 0 failures** — 456 from before plus 5 new tests × 3 projects (picker, Reported section, super admin view, bell deep link, reported places) in `messages.spec.ts` and 2 × 3 in `places.spec.ts`, minus the 2 removed `/reports/` tests |
