...
Home, gated on the account's real role (`me.role === 'member'`) — see D-161
...
unread status from `messages` directly instead (D-160) — correct today,
...
D-159, including exactly which policies to extend and how. This is the one
...
kB → 500.8 kB gz. See D-162. The honest fix (splitting locale bundles) is out
of scope here; flagged rather than absorbed silently.

## Decisions

- **D-159** — messaging is scoped to accepted `connections`, enforced by the
...
- **D-160** — `conversations.last_message_at` is dead; the list derives
  recency and unread state from `messages` directly, and does not scale past
  a small message count per conversation.
- **D-161** — Messages does not honour the super-admin role-preview system;
  ...
  account.
- **D-162** — a ~0.1 kB bundle-budget regression from this session's new
...
| `node scripts/check-bundle-budget.mjs` | 500.8 kB gz — over budget by 0.8 kB (was 0.7 kB; see D-162). `/messages` and `/messages/thread` each carry ~6.3 kB of their own route code, not counted against the shared-chunk budget this script measures, since nothing else in the app imports from them |
...
- **D-159's gap.** `conversation_members_insert`/`conversations_insert_participant`
...
- **`conversations.last_message_at` still writes nothing.** D-160 documents
...
- **The §12 budget regression (D-162)** is disclosed, not fixed.

## Needs a human

- **D-159, explicitly**: is client-side connection-scoping (with the
