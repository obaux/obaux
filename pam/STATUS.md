...
| 27 | **Approve the new transparency line, then deploy `0067`** | The ring for a new save (D-198) lights only once `people_activity()` exists live; until then the strip shows with message rings only | en: *"When you save a new place — not which one. A program you joined sees this too."* · es: *"Cuando guarda un lugar nuevo — no cual. Un programa donde se inscribio tambien lo ve."* (D-199). A changed word goes into `transparency.ts` and `en.json`/`es.json` together, or `copy.test.ts` fails. Then `list_migrations`, apply `0067_people_activity.sql`, `get_advisors`. |
