...
| Browser a11y + theme (Playwright, full suite) | **474 pass** (21 September; `PLAYWRIGHT_CHROMIUM_PATH=/opt/pw-browsers/chromium-1194/chrome-linux/chrome` — the sandbox's own Chromium, no download needed) | No WCAG AA violations at 320px or iPhone SE. Every control clears 48px. No horizontal scroll. The Astryx theme really resolves. Runs in dark mode as well as light. Includes the new `e2e/messages.spec.ts` (thread, example thread, report, `/reports/`). The 17th's `admin.spec.ts` failures from D-175's unstubbed `enrollments` query are fixed in the spec. |
