-- 0054 — A conversation partner can see who they are talking to.
--
-- Messaging in PAM is staff-to-member, corrected from an earlier, wrong build
-- of member-to-member chat (see DECISIONS.md D-152, superseding D-148/149/150).
...
-- named in D-152 for the still-open question of who may start one).
...
comment on policy profiles_select_conversation_partner on public.profiles is
  ...
  'manager''s or program''s name, not just the reverse (0054). Exposes the '
  'whole row under RLS, including `phone`; nothing in apps/web selects it for '
  'this purpose today, and D-152 flags column-level tightening (the same '
